...
const commands = {
  linux: {
    code: "curl -fsSL https://deplexo.github.io/cli/install.sh | sh",
    shell: "sh / bash / zsh",
    detail:
  ...
  windows: {
    code: "& ([scriptblock]::Create((Invoke-RestMethod -ErrorAction Stop 'https://deplexo.github.io/cli/install.ps1')))",
    shell: "PowerShell",
    detail:
...
fetch("https://api.github.com/repos/Deplexo/cli/releases/latest", {
  ...
  .then(async (response) => {
    if (response.status === 404) {
      releaseStatus.textContent = "First release in preparation ↗";
      document
